...
import com.riffdeck.player.coil.CoilBitmapLoader
import com.riffdeck.player.core.appwidgets.RiffDeckGlanceWidget
import com.riffdeck.player.core.appwidgets.CardWidget
import com.riffdeck.player.core.appwidgets.FullWidget
import com.riffdeck.player.core.appwidgets.WidgetTheme
import com.riffdeck.player.core.appwidgets.state.PlaybackState
import com.riffdeck.player.core.appwidgets.state.PlaybackStateDefinition
import com.riffdeck.player.core.audio.AudioOutputObserver
import com.riffdeck.player.core.model.player.MetadataField
import com.riffdeck.player.core.palette.PaletteProcessor
import com.riffdeck.player.data.local.MediaStoreObserver
import com.riffdeck.player.data.local.ReplayGainTagExtractor
import com.riffdeck.player.data.local.repository.Repository
import com.riffdeck.player.data.model.Song
import com.riffdeck.player.data.model.network.NetworkFeature
import com.riffdeck.player.data.model.network.ScrobblingService
import com.riffdeck.player.extensions.isBluetoothA2dpConnected
import com.riffdeck.player.extensions.isBluetoothA2dpDisconnected
import com.riffdeck.player.extensions.showToast
import com.riffdeck.player.playback.equalizer.EqualizerManager
import com.riffdeck.player.playback.library.LibraryProvider
import com.riffdeck.player.playback.library.MediaIDs
...
import com.riffdeck.player.playback.renderer.AlacWorkaroundCodecSelector
import com.riffdeck.player.playback.renderer.RiffDeckRenderersFactory
import com.riffdeck.player.ui.screen.MainActivity
import com.riffdeck.player.util.CLEAR_QUEUE_ON_COMPLETION
import com.riffdeck.player.util.ENABLE_HISTORY
import com.riffdeck.player.util.IGNORE_AUDIO_FOCUS
import com.riffdeck.player.util.MP3_INDEX_SEEKING
import com.riffdeck.player.util.PAUSE_ON_ZERO_VOLUME
import com.riffdeck.player.util.PLAY_ON_STARTUP_MODE
import com.riffdeck.player.util.PlayOnStartupMode
import com.riffdeck.player.util.Preferences
import com.riffdeck.player.util.Preferences.requireString
import com.riffdeck.player.util.QUEUE_NEXT_MODE
import com.riffdeck.player.util.REWIND_WITH_BACK
import com.riffdeck.player.util.SEEK_INTERVAL
import com.riffdeck.player.util.STOP_WHEN_CLOSED_FROM_RECENTS
import com.riffdeck.player.util.SongPlayCountHelper
import com.riffdeck.player.util.WIDGET_DYNAMIC_COLORS
import com.riffdeck.player.util.WIDGET_IMAGE_CORNER_RADIUS
import com.riffdeck.player.util.WIDGET_SMALL_LAYOUT_STYLE
...
class PlaybackService :
    ...
    private val playerThread = HandlerThread("RiffDeck-ExoPlayer", Process.THREAD_PRIORITY_AUDIO)
    ...
    override fun onCreate() {
        ...
        player = AdvancedForwardingPlayer(
            ExoPlayer.Builder(this)
                ...
                .setRenderersFactory(
                    RiffDeckRenderersFactory(this, balanceProcessor, replayGainProcessor)
    ...
    private suspend fun updateGlanceWidgets(playbackState: PlaybackState) = withContext(IO) {
        try {
            val riffDeckWidget = RiffDeckGlanceWidget()
            val riffDeckWidgetIds = glanceManager.getGlanceIds(riffDeckWidget.javaClass)
            if (riffDeckWidgetIds.isNotEmpty()) {
                riffDeckWidgetIds.forEach { id ->
                    updateAppWidgetState(applicationContext, PlaybackStateDefinition, id) {
                        playbackState
                    }
                    riffDeckWidget.update(applicationContext, id)
                }
            }
    ...
    companion object {
        private const val PACKAGE_NAME = "com.riffdeck.player"
