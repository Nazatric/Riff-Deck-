...
import com.riffdeck.player.core.model.lyrics.LyricsViewSettings.Key
...
import java.io.File
import com.riffdeck.player.core.model.lyrics.LyricsViewSettings.Mode as LyricsViewMode
...
class LyricsViewModel(
    ...
    /**
     * Manually (re-)fetches lyrics for [song] from the online providers
     * (LRCLIB, BetterLyrics, SimpMusic — see LyricsDownloadService),
     * bypassing any prior userCleared state so a manual retry always
     * attempts a fresh download. Used by the "Add lyrics" / "Retry" action
     * shown when no lyrics are found or the current ones are wrong.
     */
    fun retryLyrics(song: Song) {
        if (song == Song.emptySong) return
        lyricsJob?.cancel()
        _lyricsResult.value = LyricsResult(id = song.id, loading = true)
        lyricsJob = viewModelScope.launch {
            withContext(Dispatchers.IO) {
                // Clear only this song's stored lyrics/userCleared flag
                // first, so allLyrics()'s (!storedLyrics.userCleared) guard
                // doesn't skip the network fetch on this explicit retry —
                // scoped per-song, unlike the global "Clear lyrics" Settings
                // action which intentionally wipes the whole library.
                runCatching { lyricsRepository.deleteLyrics(song.id) }
            }
            val lyrics = withContext(Dispatchers.IO) {
                runCatching {
                    lyricsRepository.allLyrics(song, allowDownload = true, fromEditor = false)
