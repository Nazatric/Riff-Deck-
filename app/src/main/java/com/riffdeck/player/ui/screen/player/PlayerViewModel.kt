package com.riffdeck.player.ui.screen.player
...
import com.riffdeck.player.core.model.shuffle.ShuffleOperationState
import com.riffdeck.player.core.model.shuffle.SpecialShuffleMode
import com.riffdeck.player.core.sort.SongSortMode
import com.riffdeck.player.data.SongProvider
...
import com.riffdeck.player.data.mapper.toSongs
import com.riffdeck.player.data.model.QueuePosition
import com.riffdeck.player.data.model.Song
import com.riffdeck.player.playback.Playback
import com.riffdeck.player.playback.getQueueItems
import com.riffdeck.player.playback.progress.ProgressObserver
import com.riffdeck.player.playback.shuffle.OpenShuffleMode
import com.riffdeck.player.playback.shuffle.ShuffleManager
import com.riffdeck.player.playback.toMediaItems
import com.riffdeck.player.util.NOW_PLAYING_EXTRA_INFO
import com.riffdeck.player.util.Preferences
import com.riffdeck.player.util.REMEMBER_SHUFFLE_MODE
...
class PlayerViewModel(
    ...
    // RiffDeck: exposes the live audio session ID so the Winamp-style
    // spectrum visualizer (RiffDeckAudioVisualizer) can attach to the
    // actual playing audio. 0 means "no session" / visualizer should show
    // an idle state rather than attempt to attach.
    private val _audioSessionIdFlow = MutableStateFlow(0)
    val audioSessionIdFlow = _audioSessionIdFlow.asStateFlow()
    ...
    fun setMediaController(mediaController: MediaController?) {
        ...
        if (mediaController != null) {
            ...
            // Note: audioSessionIdFlow is populated exclusively via the
            // onAudioSessionIdChanged(Int) listener callback below, not
            // read eagerly here. That callback is guaranteed to fire once
            // a session is assigned (confirmed part of Player.Listener),
            // whereas a direct synchronous getter isn't relied upon.
    ...
    override fun onAudioSessionIdChanged(audioSessionId: Int) {
        _audioSessionIdFlow.value = audioSessionId
    }
