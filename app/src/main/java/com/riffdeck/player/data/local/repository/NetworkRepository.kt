package com.riffdeck.player.data.local.repository
...
import com.riffdeck.player.data.local.room.ArtistInfoDao
import com.riffdeck.player.data.local.room.ArtistInfoEntity
import com.riffdeck.player.data.model.Song
import com.riffdeck.player.data.model.network.LoginParams
import com.riffdeck.player.data.model.network.LoginState
import com.riffdeck.player.data.model.network.ScrobblingResult
import com.riffdeck.player.data.model.network.ScrobblingService
import com.riffdeck.player.data.model.network.lastfm.LastFmFailure
import com.riffdeck.player.data.remote.deezer.DeezerService
import com.riffdeck.player.data.remote.deezer.model.DeezerAlbum
import com.riffdeck.player.data.remote.deezer.model.DeezerArtist
import com.riffdeck.player.data.remote.deezer.model.DeezerTrack
import com.riffdeck.player.data.remote.lastfm.LastFmService
import com.riffdeck.player.data.remote.lastfm.model.LastFmAlbum
import com.riffdeck.player.data.remote.lastfm.model.LastFmArtist
import com.riffdeck.player.data.remote.lastfm.model.LastFmError
import com.riffdeck.player.data.remote.lastfm.model.LastFmSessionResponse
import com.riffdeck.player.data.remote.lastfm.model.LastFmUser
import com.riffdeck.player.data.remote.lastfm.model.NowPlayingResponse
import com.riffdeck.player.data.remote.lastfm.model.ScrobbleResponse
import com.riffdeck.player.data.remote.listenbrainz.ListenBrainzService
import com.riffdeck.player.data.remote.listenbrainz.model.ListenBrainzListen
import com.riffdeck.player.data.remote.listenbrainz.model.ListenBrainzSubmission
import com.riffdeck.player.data.remote.listenbrainz.model.ListenBrainzTrackAdditionalInfo
import com.riffdeck.player.data.remote.listenbrainz.model.ListenBrainzTrackMetadata
import com.riffdeck.player.extensions.media.displayArtistName
import com.riffdeck.player.util.CryptoUtil
...
class NetworkRepositoryImpl(
    ...
    override suspend fun artistInfo(name: String, lang: String?, cache: String?): LastFmArtist? {
        // RiffDeck: try the network first when online (keeps existing
        // behavior/freshness), but on any failure — including being
        // offline — fall back to the last successfully cached biography
        // for this artist instead of returning null and showing nothing.
        ...
        val bioContent = networkResult?.artist?.bio?.content
        if (!bioContent.isNullOrBlank()) {
            runCatching {
                artistInfoDao.insertArtistInfo(
                    ArtistInfoEntity(
                        artistName = name,
        ...
        // Network failed or returned no bio (e.g. offline) — try the cache.
        val cached = runCatching { artistInfoDao.getArtistInfo(name) }.getOrNull()
        if (cached != null) {
            return LastFmArtist(
                artist = LastFmArtist.Artist(bio = LastFmArtist.Bio(content = cached.biography))
            )
        }
